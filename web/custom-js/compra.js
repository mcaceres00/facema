...
function openPopUp(){
  var pop = window.open('?View=Proveedor', '_blank', 'location=yes,height=570,width=820,scrollbars=yes,status=yes');
  pop.onbeforeunload = function(){
    var urlprocess = 'web/ajax/listarproveedor.php';
    $.ajax({
      type:'POST',
      url:urlprocess,
      dataType: 'json',
      success: function(result){
        ...
        result.map(function(item){
          $('#cbProveedor').append('<option value = "'+item[0]+'">'+item[2]+'</option>');
        });
      }
    });
  }
