<?php
	...
	class CajaModel extends Conexion
	{
		...
		public function Listar_Datos_Fecha($fecha)
		{
			$dbconec = Conexion::Conectar();

			try
			{
				$query = "CALL sp_view_datos_caja_fecha(:fecha);";
				$stmt = $dbconec->prepare($query);
		...
		public function Listar_Movimientos_Fecha($fecha)
		{
			$dbconec = Conexion::Conectar();

			try
			{
				$query = "CALL sp_view_movimientos_caja_fecha(:fecha);";
				$stmt = $dbconec->prepare($query);
				$stmt->bindParam(":fecha",$fecha);
				$stmt->execute();
				$count = $stmt->rowCount();
		...
		public function Listar_Movimientos_Detalle_Fecha($fecha)
		{
			$dbconec = Conexion::Conectar();

			try
			{
				$query = "CALL sp_view_movimientos_detalle_fecha(:fecha);";
				$stmt = $dbconec->prepare($query);
				$stmt->bindParam(":fecha",$fecha);
				$stmt->execute();
				$count = $stmt->rowCount();
		...
		public function Abrir_Caja($monto)
		{
			...
			try
			{
				/*$queryLastId = "SELECT MAX(idcaja)+1 as lastId FROM caja";
				$stmt = $dbconec->prepare($queryLastId);
				$stmt->execute();
				$lastId = $stmt->fetch(PDO::FETCH_ASSOC);
				//var_dump($lastId['lastId']);
				//die;
				$query = "CALL sp_abrir_caja(:monto, :lastId)";
				$stmt = $dbconec->prepare($query);
				$stmt->bindParam(":monto",$monto);
				$stmt->bindParam(":lastId", intval($lastId['lastId']));*/
				//var_dump($monto);
