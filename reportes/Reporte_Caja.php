...
// Page footer
    ...
    try {
      ...
      $objCaja =  new Caja();
    	...
    	$caja = $objCaja->Listar_Datos_Fecha($fecha);
    	$totales = $objCaja->Listar_Movimientos_Fecha($fecha);
    	$movimientos = $objCaja->Listar_Movimientos_Detalle_Fecha($fecha);
      ...
      $pdf->setX(140);
      $pdf->Cell(40,7,'Numero de Cierre:',0,0,'L',1);
      $pdf->Cell(20,7,'',0,0,'L',1);
      ...
      if (is_array($movimientos) || is_object($movimientos)){
          foreach ($movimientos as $row => $column) {
            if ($column['tipo_movimiento'] <= 2) {
              ...
              $pdf->ln();
              $id_caja = $column['idcaja'];
              $total_ingresos = $total_ingresos + $column['monto_movimiento'];
            }
          }
      }

      $pdf->SetFillColor(230,230,230);
      $pdf->Cell(140,7,'Total Ingresos',1,0,'L',1);
      $pdf->Cell(50,7,$total_ingresos,1,0,'R',1);
      $pdf->SetFillColor(255,255,255);
      $pdf->ln(15);

      $pdf->Cell(140,7,'Saldo de Caja',1,0,'L',1);
      $pdf->Cell(50,7,$total_ingresos - $total_egresos,1,0,'R',1);
      $pdf->ln();
      $pdf->Cell(140,7,'Monto de Cierre',1,0,'L',1);
      $pdf->Cell(50,7,$monto_apertura + ($total_ingresos - $total_egresos),1,0,'R',1);
      $pdf->ln(20);
